...
const ChemQuizList = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      setLoading(true);
      const res = await getChemistryByTag(selectedTag);
      setData(res);
      setLoading(false);
    };
    fetch();
    ...
    <div className="min-h-screen p-4 sm:p-8 max-w-7xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="flex justify-center items-center h-64 text-white/60 text-lg">
          Please select a tag above to load chemistry quizzes.
        </div>
      ) : (
          ...
          {data.length > 0 ? (
            data.map((item) => (
               <PracticeCard 
                 key={item.id}
                 practice={item} 
                 onClick={() => navigate(`/chem-quiz/${item.id}${selectedTag ? `?tag=${selectedTag}` : ''}`)} 
               />
            ))
          ) : (
